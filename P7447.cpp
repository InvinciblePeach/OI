...
void pushdown_bl(int bl, int L, int R, ll &x) {
    if (!x) return;
    for (int i = L; i <= R; i++)
...
void query_bl(int bl, int L, int R, ll &ans, int &mx, int &mn) {
    for (int i = L; i <= R; i++)
        if (lb[bl] <= a[i] && a[i] <= rb[bl])
...
void query(int bl, int p, int l, int r, ll &ans, int &mx, int &mn) {
    if (l <= t[bl][p].l && t[bl][p].r <= r) {
        ans += t[bl][p].val, mx = max(t[bl][p].mx, mx), mn = min(t[bl][p].mn, mn);
        return;
    }
    ...
    if (t[bl][p].r - t[bl][p].l + 1 <= K) {
        pushdown_bl(bl, t[bl][p].l, t[bl][p].r, t[bl][p].tag);
        l = max(l, t[bl][p].l), r = min(r, t[bl][p].r);
        query_bl(bl, l, r, ans, mx, mn);
        return;
    }

    int mid = (t[bl][p].l + t[bl][p].r) >> 1;
    pushdown(bl, p);
    if (l <= mid) query(bl, ls(p), l, r, ans, mx, mn);
    if (r > mid) query(bl, rs(p), l, r, ans, mx, mn);
    pushup(bl, p);
}
...
signed main() {
    ...
    while (m --> 0) {
        ...
        if (op == 1) {
            ...
            ll ans = 0;
            int mx = 0, mn = INT_MAX;
            for (int i = 1; i < 9; i++) query(i, 1, l, r, ans, mx, mn);
            io.write(ans, ' '), io.write(mn, ' '), io.write(mx, '\n');
            lst = ans & mod;
        }
    }
    return 0;
